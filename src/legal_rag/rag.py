...
class LegalRagEngine:
    def __init__(
        ...
        self.generator = load_generator_if_available(model_path)

    @staticmethod
    def _build_prompt(query: str, lang: str, chunks: List[Dict[str, str]]) -> str:
        if lang == "ar":
            instruction = (
    ...
    def answer(self, query: str, k: int = 5, min_score: float = 0.35, max_tokens: int = 512) -> Dict[str, object]:
        ...
        if self.generator is not None:
            prompt = self._build_prompt(query=query, lang=lang, chunks=chunks)

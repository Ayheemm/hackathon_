...
export default function CalculatorsHomePage() {
  return (
    <main className="min-h-screen bg-[var(--bg-main)] px-4 py-8 text-[var(--text-dark)] md:px-8">
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-2">
          ...
          <Link
            href="/chat"
            className="inline-flex items-center gap-1 rounded-lg border border-[rgba(212,160,80,0.33)] bg-transparent px-3 py-2 text-sm font-semibold text-[var(--text-dark)] transition hover:bg-[rgba(212,160,80,0.07)]"
          >
            Retour au chat
          ...
          {cards.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="group rounded-[12px] border border-[0.5px] border-[var(--border-gold)] bg-[var(--bg-card)] p-5 shadow-soft transition hover:-translate-y-0.5 hover:bg-[rgba(212,160,80,0.07)]"
              >
                <div className="mb-3 inline-flex rounded-[10px] border border-[0.5px] border-[var(--border-gold)] bg-[var(--bg-main)] p-2 text-[var(--bg-mid)]">
                  <Icon className="h-5 w-5" aria-hidden="true" />
                </div>
                <h2 className="font-fr text-lg font-semibold text-[var(--text-dark)]">{item.title}</h2>
                <p className="mt-1 text-sm text-[var(--bg-dark)]">{item.subtitle}</p>
